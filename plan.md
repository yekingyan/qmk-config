...
- [ ] **失声问题长期验证中**（已 **8天21小时49分**无 USB 失声、无重新枚举、无复位；08-17 有过一次主机发起的总线复位，不足 2 秒自愈，非 E15，见「观察期进度」）：观察起点 **2026-08-11 13:23:38**（端口也从 `Port_#0006` 换到了 `Port_#0003`），需累计 2~3 周**实际使用**无「不可恢复失声」
...
| 2026-08-20 11:13 | **一周复查：8天21小时49分无 USB 失声**。`USBDIAG a=1 b=0 heal=1 rst=0 mcu=0 st=2 up=769794s`，与 08-17 读数相比除 `up` 外完全没变 → 那次之后又平静 2天19小时。`up` 反推开机时刻与主机 `LastArrivalDate=2026-08-11 13:23:38` 仅差 6 秒（按键/消息时刻误差）→ 无 MCU 复位。主机枚举时刻未变、D0、零 USB/HID 错误、零休眠事件；watcher PID 310252 仍活着且 `.kb-state` 在推进。观察期达目标的 **42~64%** |
| 2026-08-17 16:09 | **首个真实事件**：`a=1 b=0 heal=1 rst=0 mcu=0 st=2 up=528364s`。`st=2`=`USB_READY`（*"Ready, after bus reset"*）→ 主机发起过一次总线复位，2 秒内自愈、未升到第 2 级。`b=0` → **不是 E15**。主机侧枚举时刻未变（端口重置恢复，未拆 PnP 栈）、零 USB/HID 错误、零休眠事件、D0。诊断链路首次通过实战检验；同时暴露一个残留风险：主机发起的复位若超过 2 秒会被第 2 级打断 |
...
| **2026-08-11 13:23:38** | 进行中 | **已 8天21小时49分无 USB 失声**（08-17 一次主机总线复位，不足 2 秒自愈）| — | — | **✅ 完整修复版（`843a677`）的观察起点。** 端口 `Port_#0003`。截至 08-20：`b=0 rst=0 mcu=0`，主机 `LastArrivalDate` 从刷机起未变；没有 E15 证据、没有第 2/3 级恢复、没有重新枚举 |
...
| **2026-08-17 16:09** | **146h46m**（6.11 天）| **`a=1 b=0 heal=1 rst=0 mcu=0 st=2 up=528364s`** | **首个真实事件！但不是 E15** —— 见下方专节。用户补充「今天早上还是都是 0 的」→ 事件发生在 08-17 白天 |
| **2026-08-20 11:13** | **213h49m**（8.91 天）| **`a=1 b=0 heal=1 rst=0 mcu=0 st=2 up=769794s`** | 除 `up` 外与 08-17 完全相同 → 此后 2天19小时没有新事件；从刷机起无 USB 失声、无重新枚举、无 MCU 复位。`up` 与主机枚举时刻在 6 秒内对账一致 |

### 🔍 首个真实事件（2026-08-17）：主机发起的总线复位，2 秒内自愈

`a=1 b=0 heal=1 rst=0 mcu=0 st=2 up=528364s`

**逐项判读：**

| 读数 | 含义 |
|------|------|
| `st=2` | `USB_READY`，ChibiOS 注释原文 *"Ready, after bus reset"*（`hal_usb.h`）|
| `a=1` | 条件(a) 命中一次：`state != USB_ACTIVE` |
| `b=0` | 帧号**从未**停滞 → **不是 E15** |
| `heal=1` `rst=0` | **在升到第 2 级之前就自己好了** —— 卡住不足 2 秒，`restart_usb_driver()` 一次没调 |
| `mcu=0`、`up` 未归零 | 没有芯片复位 |

**状态是怎么变成 `USB_READY` 的**（查实的完整链路）：

```
USB_INTS_BUS_RESET 中断         chibios-contrib .../RP/LLD/USBDv1/hal_usb_lld.c:370
  └─ _usb_reset(usbp)                                                        :373
       └─ usbp->state = USB_READY   // =2    chibios/os/hal/src/hal_usb.c
```

之后主机重新分配地址（→ `USB_SELECTED`=3）、重新选择配置（→ `USB_ACTIVE`=4）。
我们的检测器是在 `USB_READY` 这个中间态抓到的。

**主机侧的对应证据（全部指向「端口重置恢复」而非重新枚举）：**

| 检查项 | 结果 | 说明 |
|-------|------|------|
| 枚举时刻 | **未变**（仍 `2026-08-11 13:23:38`）| PnP 设备栈没被拆掉 → 所以也没响提示音、没弹窗 |
| USB/HID 错误日志 | 无 | |
| 休眠/唤醒事件 | 无 | 排除休眠唤醒诱因 |
| 电源状态 | D0 | 排除选择性挂起 |
| `USBHUB3`/`USBXHCI` 操作日志 | **本机不存在该日志** | 所以拿不到主机侧的时刻与原因，这是本次调查的硬性限制 |

结论：Windows 做了一次 **端口重置恢复**（reset + 重新寻址 + 重新配置，不拆 PnP 栈），
设备被打回 `USB_READY`，不到 2 秒就回到 `USB_ACTIVE`。

**这次事件的价值（三条，都不是「修复被证明有效」）：**

1. ✅ **诊断链路首次通过实战检验**：计数器递增、`diag_last_state` 抓到了状态、
   `diag_selfhealed` 分支真实执行过。这些代码此前一行没跑过。
2. ✅ **检测器没有过激反应**：看到异常但正确地放手让它自愈，没有触发第 2 级那种
   破坏性动作。这是门控与「每周期只做一次」逻辑的首次真实验证。
3. ⚠️ **说明总线错误确实在发生**：Windows 通常是因为传输错误才去重置端口。
   而 E15 的触发因子恰恰是「IN 传输期间的**总线错误**」。
   这次错误被主机自己恢复了，但**触发 E15 的前置条件在这套硬件上确实存在**。
   不能据此说 E15 会发生，但也不能再说「这套环境很干净」。

**观察窗口不中断**：这不是「不可恢复失声」，2~3 周的计时继续。

### ⚠️ 本次事件暴露的残留风险：主机发起的总线复位可能被我们打断

这次侥幸在 2 秒内完成了。但逻辑上存在一个我们没防住的情况：

**如果主机发起的总线复位 + 重新配置耗时超过 2 秒，第 2 级就会开火**，
`restart_usb_driver()` 会把主机正在进行的合法重新配置掐断 ——
这正是 08-11 那个「自伤」模式，只不过起因从「我们自己的枚举」换成了「主机的复位」。
5 秒宽限期只保护前者，**不保护后者**。

而 `rst=0` 只告诉我们这次不足 2 秒，**没告诉我们离 2 秒有多近**，余量未知。

**候选改法（待用户决定，需刷机）：**

| # | 改法 | 理由 |
|---|------|------|
| 1 | `state` 为 `USB_READY(2)` 或 `USB_SELECTED(3)` 时**不升到第 2 级** | 这两个状态明确表示「主机正在重新配置我们」。此时 `restart_usb_driver()` 只可能把事情弄糟 —— 主机已经在复位我们了。真正的 E15 特征是 `state == USB_ACTIVE` 且帧号停滞，与这两个状态互斥 |
| 2 | 增记「历史最长卡住时长」计数 | 才能知道离 2 秒阈值有多少余量，否则永远是盲的 |

**进度**：8天21小时49分 / 目标 2~3 周 ≈ **42~64%**。

**这些数据怎么准确表述？**

- ✅ 可以说：**「完整修复版已连续运行超过一周，没有发生 USB 失声、重新枚举、
  第 2 级 USB 重启或第 3 级 MCU 复位。」** `b=0 rst=0 mcu=0`，且主机
  `LastArrivalDate` 从 08-11 13:23:38 起未变。
- ⚠️ 不能说「整周完全零事件」：08-17 有过一次 `USB_READY(2)`，说明主机发起过
  总线复位；它在 2 秒内自行恢复（`heal=1`），用户没有报告可感知的 USB 失声。
  从那次事件到本次复查又平静了 **2天19小时**。
- ⚠️ 也不能说「自愈修复已经证明有效」：`rst=0 mcu=0` 表明第 2/3 级至今没执行过；
  这 8.9 天只证明没有遇到需要它们处理的故障。

**这些零事件有多少说服力？**

- 历史失声间隔是「**7~8 小时到很多天**」，08-10 那次实测存活 ≤7h57m。
  8.91 天已明显超过已知短间隔，开始有意义；但用户明确说过
  「有时很多天都是正常，忽然有一天就有问题」，所以仍可能只是成簇故障之间的安静期。
- 截至目前 **`b=0`**：帧号从未停滞，仍没有抓到 E15。唯一一次 `a=1/heal=1`
  是主机总线复位后不足 2 秒自行恢复，不能算 E15，也没有触发我们的第 2/3 级恢复。
- ⚠️ **不能只归功于固件**：刷机时同时换了 USB 口（`Port_#0006` → `Port_#0003`）。
  E15 的触发因子是「IN 传输期间的总线错误」，物理层改善会直接降低发生率；
  本次观察无法区分固件与换口的贡献。
- ⚠️ **休眠路径仍未被测到**：截至 08-20 Kernel-Power 42/107 仍是零记录，
  电脑没有睡眠/唤醒事件。因此这条路径不能算已验证。


### ⚠️ 成簇发生：为什么「零事件」是弱证据（2026-08-13 用户确认）

用户明确描述过发生模式：**「有时很多天都是正常，忽然有一天就有问题。」**

这和 E15 的触发机制吻合 —— 触发因子是「IN 传输期间的**总线错误**」，
而总线错误本身就受环境影响（线材姿态、附近干扰源、主机负载），
所以是**成簇**而非均匀随机。08-11 上午 20 小时内 6 次成功枚举 + 4 次失败枚举、
间隔一度缩到 48 秒，就是一个典型的簇。

**由此推出三条，会改变结案判据：**

1. **连续零事件的说服力比直觉低得多。** 44 小时清白完全可能只是落在两个簇之间的
   安静期。既然历史上有「很多天正常」的先例，2~3 周无事件也未必能排除。
2. **我们其实需要它发生一次。** 零事件只能证明「没触发」，**不能证明「修复有效」**。
   真正的强证据是「事件发生了 → 自动恢复了 → 你几乎没察觉」。
   08-11 10:18 拿到过一次这种证据，但那是**有自伤缺陷的 `0acbfeb`** 版本。
3. **截至 08-13，当前固件的恢复路径一次都没跑过。** 当时自伤修复的宽限期、门控拆档、Sticky master、
   `watchdog_reboot` 全是「只在紧急时刻才执行」的未验证代码。08-17 的真实事件后来让
   **检测 + `diag_selfhealed` 分支**首次通过实战检验（`heal=1`），但
   **第 2 级 `restart_usb_driver()` 与第 3 级 `watchdog_reboot()` 至今仍未执行**
   （`rst=0 mcu=0`）；所以自伤宽限期、Sticky master 与整芯片复位路径仍未经过当前固件的实战验证。

**结论：零事件是弱证据，但用户选择继续被动观察。** 见下方决策。

### ❌ 已否决（2026-08-13）：主动注入假失声来验证恢复阶梯

> **用户决定：「不了，就真实使用吧。无非耗点时间。」**
> **这是已定决策，不要再提。** 继续纯被动观察，靠真实使用积累证据。

留档一下当时的提案与权衡，以免以后有人重复想到：

**提案做法**：加测试键码，按下后让 `usb_is_dead()` 在一段时间内强制返回真
（第 2 级本身就造成真实重新枚举，所以窗口够长时整条阶梯会自然走完）。
分两步：3 秒验证第 1~2 级（低风险），12 秒验证第 3 级
`watchdog_reboot()` + Sticky master（中风险，兜底是拔插一次 USB —— 断电后
`watchdog_hw->reason` 归 0、`scratch[1]` 清空，走正常主从判定；Bootmagic 也仍可用，
所以最坏情况是拔插一次，不会变砖）。

**为什么否决是合理的**：注入测试能把「未验证的紧急代码」变成「已验证」，
但它验证的是**人造条件下的恢复阶梯**，而不是 E15 真实锁死时的行为 ——
后者才是真问题。而且它必然要付一次刷机 + 一次真实的重新枚举。
用户既然不介意等，被动观察的证据质量其实**更高**（真实触发条件、真实现场数据），
只是拿到的时间不确定。

**因此接受的代价（写明以便日后判读）**：

- 第一次真实失声发生时，恢复阶梯是**首次执行**。若 Sticky master 或
  `watchdog_reboot` 有 bug，会在那个时刻暴露，可能需要拔插一次 USB。
- 这也是为什么「第一次真实事件」的现场数据格外宝贵：
  **卡了多久、有没有弹「USB 不识别」、`USB_DIAG` 的 `a`/`b`/`st`/`mcu`**，
  这四项决定了我们是「修复有效」还是「还得改」。届时务必先按 `USB_DIAG` 再做别的。
...
> ## ⚠️ 第一件事：先按 `USB_DIAG`，别先做别的
>
> 位置：`_FUN` 层左上角 `QK_BOOTLOADER` **正下方**（矩阵 `[1,0]`）。
>
> 2026-08-13 已决定**不做人工注入测试**，靠真实使用积累证据。
> 代价是：**第一次真实失声时，恢复阶梯是首次执行**，那一刻的现场数据不可替代。
> 一旦你拔了线、或者重启了电脑，`up` 归零、计数器清零（`mcu` 除外），证据就没了。
>
> 需要的四项，按重要性排序：
>
> | 项 | 怎么拿 | 决定什么 |
> |----|-------|---------|
> | 1. `USB_DIAG` 四个计数 | 按键打出来，**发给我原文** | `b` 涨而 `a` 不涨且 `st=4` → **E15 被证实**；`a` 涨 → 是软件状态机问题 |
> | 2. 卡了多久 | 你的体感 | 约 2 秒 = 第 2 级救回；约 10 秒 = 第 3 级；一直不好 = 情况 B |
> | 3. 有没有弹「USB 不识别」 | 屏幕右下角 | **弹了 = 自伤修复失败**（修复前那 4 次全是自伤，修复后不该再出现）|
> | 4. `mcu` 是否 > 0 且 `up` 是否归零 | 同第 1 项 | 第 3 级动过 → Sticky master 与 `watchdog_reboot` 首次通过实战检验 |
>
> **如果键盘完全没救回来**（情况 B）：先别拔线，按几个键、等满 6 秒
> 让第 3 级有机会执行；仍不行再拔插，然后立刻跑 `scripts/kb-diag.sh --history`。
