...
- [ ] **失声问题长期验证中**（已 6.11 天；08-17 有过一次主机发起的总线复位，2 秒内自愈，非 E15，见「观察期进度」）：观察起点 **2026-08-11 13:23:38**（端口也从 `Port_#0006` 换到了 `Port_#0003`），需累计 2~3 周**实际使用**无「不可恢复失声」
...
| **2026-08-11 13:23:38** | 进行中 | **已 6.11 天**（其中 08-17 一次主机总线复位，自愈）| — | — | **✅ 完整修复版（`843a677`）的观察起点。** 端口 `Port_#0003`。连续复查 3 次全零（详见「观察期进度」），每次 `up` 反推开机时刻均 = `13:23:38`，与主机侧**完全一致** |
...
3. **当前固件的恢复路径一次都没跑过。** 自伤修复的宽限期、门控拆档、Sticky master、
   `watchdog_reboot` —— 全是「只在紧急时刻才执行」的代码，至今 `heal=0`，
   意味着这些路径**从未被真实执行验证过**。其中 Sticky master 和 `watchdog_reboot`
   一旦有 bug，后果是键盘复位后不回来（见「⚠️ 新发现的风险」一节）。
