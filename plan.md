...
- [ ] **失声问题长期验证中**（已 20h46m 零事件，见「观察期进度」）：观察起点 **2026-08-11 13:23:38**（端口也从 `Port_#0006` 换到了 `Port_#0003`），需累计 2~3 周**实际使用**无「不可恢复失声」
...
| **2026-08-11 13:23:38** | 进行中 | **已 20h46m 无事件** | — | — | **✅ 完整修复版（`843a677`）的观察起点。** 端口 `Port_#0003`。08-12 10:10 复查：`USBDIAG a=0 b=0 heal=0 rst=0 mcu=0 st=255 up=74787s`，`up` 反推开机时刻 = `13:23:38`，与主机侧**完全一致** |
...
- 历史失声间隔是「**7~8 小时到几天**」，08-10 那次实测存活 ≤7h57m。
  所以 20h46m 已经**超过历史区间的下沿约 2.6 倍** —— 有意义，但远没到上沿。
- 更值得注意的不是「没重新枚举」，而是 **`heal=0`：检测器一次都没响过**。
  即这 20 小时里连一次「帧号停滞 >100ms」的瞬态都没有。修复前的 08-11 上午，
  同样时长内发生了 6 次成功重新枚举 + 4 次失败枚举，间隔一度缩到 48 秒。
- ⚠️ **但不能只归功于固件**：同时换了 USB 口（`Port_#0006` → `Port_#0003`）。
  E15 的触发因子是「IN 传输期间的总线错误」，物理层改善会直接降低发生率。
  两个变量同时改变，这次观察无法区分二者的贡献。
- 这一夜**电脑没有休眠**（Kernel-Power 42/107 零记录），所以
  「休眠/唤醒后 USB 是否正常」这条路径本次**没有被测到**，不能算已验证。

距结案目标（2~3 周实际使用）约 **4~6%**。
