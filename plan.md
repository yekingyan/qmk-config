...
**结论：不该只被动等。** 应该主动注入一次假失声，在**可控时刻**把整条恢复阶梯
跑通一遍，把「紧急时才跑的未验证代码」变成「已验证代码」。方案见下。

### 提案（待用户决定）：主动注入假失声，验证恢复阶梯

**做法**：加一个测试键码，按下后让 `usb_is_dead()` 在一段时间内强制返回真：

```c
static uint32_t force_dead_until = 0;   // 测试用：强制判死到此刻为止
// usb_is_dead() 开头插入：
if (force_dead_until && timer_elapsed32(force_dead_until) > (UINT32_MAX / 2)) return true;
```

因为第 2 级本身就会造成真实重新枚举（`state` 必然离开 `USB_ACTIVE`），
所以只要强制窗口够长，整条阶梯会自然走完，不需要额外造假。

**分两步做，先易后险：**

| 步骤 | 强制时长 | 会跑到哪 | 验证什么 | 风险 |
|------|---------|---------|---------|------|
| A | **3 秒** | 第 1 级 → 第 2 级 → 5s 宽限期 | 日常真正依赖的那条路：卡约 2 秒后自愈。`heal`/`rst` 应 +1，主机侧重新枚举一次，**不应弹「USB 不识别」**（这直接验证自伤修复）| 低。等价于自然发生一次失声 |
| B | **12 秒** | 一直到第 3 级 `watchdog_reboot()` | Sticky master + 跨复位计数：复位后键盘**必须回来且仍是主手**，`mcu` 应 +1、`up` 归零 | 中。若 Sticky master 有 bug，键盘复位后可能不回来 |

**步骤 B 的兜底**：拔插一次 USB 即可 —— 断电后 `watchdog_hw->reason` 归 0、
`scratch[1]` 清空，走正常主从判定路径。Bootmagic（插线时按住左上角）也仍然可用。
所以最坏情况是「拔插一次」，不会变砖。

**为什么值得做**：`heal=0` 说明这些代码至今一行没跑过，而它们恰恰是
「出事时唯一指望」。尤其第 3 级涉及整芯片复位 + 主从身份沿用，
出错的后果是键盘不回来 —— 这种事**必须在可控时刻发现，不能等它在工作中发生**。

成本约 25 行代码 + 一颗测试键 + 一次刷机。
