...
- [ ] **失声问题长期验证中**（已 44h40m 零事件，见「观察期进度」）：观察起点 **2026-08-11 13:23:38**（端口也从 `Port_#0006` 换到了 `Port_#0003`），需累计 2~3 周**实际使用**无「不可恢复失声」
...
| 2026-08-13 10:04 | 复查：**44h40m 零事件**，`up=160821s` 对账一致。已是历史失声间隔下沿（7~8h）的 5.6 倍。休眠路径仍未被测到（连续 44h 电脑一次没睡）|
| 2026-08-12 10:10 | **首个整日复查：20h46m 零事件**。`USBDIAG a=0 b=0 heal=0 rst=0 mcu=0 st=255 up=74787s`；`up` 反推开机时刻精确等于主机侧枚举时刻 `13:23:38` → 无 MCU 复位、无重新枚举。watcher 活过一夜（`.kb-state` 一直在推进）。主机侧零 USB/HID 错误、零休眠事件、D0、9 接口全 OK。**已超过历史失声间隔下沿（7~8h）约 2.6 倍**，但换端口是混杂变量，且休眠路径本次未被测到 |
...
| **2026-08-11 13:23:38** | 进行中 | **已 44h40m 无事件** | — | — | **✅ 完整修复版（`843a677`）的观察起点。** 端口 `Port_#0003`。连续复查 3 次全零（详见「观察期进度」），每次 `up` 反推开机时刻均 = `13:23:38`，与主机侧**完全一致** |

### 观察期进度

| 复查时刻 | 已存活 | `USB_DIAG` 读数 | 判读 |
|---------|-------|----------------|------|
| 2026-08-11 14:16 | 52m | `a=0 b=0 heal=0 rst=0 mcu=0 st=255 up=3146s` | 基线，全零 |
| 2026-08-12 10:10 | 20h46m | `a=0 b=0 heal=0 rst=0 mcu=0 st=255 up=74787s` | 仍全零。`up` 与主机侧枚举时刻精确对账一致 → 无 MCU 复位、无重新枚举 |
| 2026-08-13 10:04 | **44h40m**（1.86 天）| `a=0 b=0 heal=0 rst=0 mcu=0 st=255 up=160821s` | 仍全零。对账一致（`up` 反推开机 = `13:23:38`）。主机侧零 USB/HID 错误、零休眠事件、枚举时刻未变 |

**这 44h40m 有多少说服力？**

- 历史失声间隔是「**7~8 小时到几天**」，08-10 那次实测存活 ≤7h57m。
  44h40m 已是下沿的 **5.6 倍**，并且进入了历史区间的上半段 —— 越来越有意义，
  但「几天」这个上沿还没跨过去，仍不能结案。
- 更值得注意的是 **`heal=0`：检测器一次都没响过**，连一次「帧号停滞 >100ms」的
  瞬态都没有。修复前的 08-11 上午，20 小时内发生了 6 次成功重新枚举 + 4 次失败枚举，
  间隔一度缩到 48 秒。
- ⚠️ **不能只归功于固件**：同时换了 USB 口（`Port_#0006` → `Port_#0003`）。
  E15 的触发因子是「IN 传输期间的总线错误」，物理层改善会直接降低发生率。
  两个变量同时改变，本次观察无法区分二者的贡献。
- ⚠️ **休眠路径至今未被测到**：连续 44 小时 Kernel-Power 42/107 零记录，
  电脑一次没睡过。所以「休眠/唤醒后 USB 是否正常」不能算已验证。

距结案目标（2~3 周实际使用）约 **9~13%**。
